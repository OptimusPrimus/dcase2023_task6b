...
class Cnn14(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin,
                 fmax, classes_num, time_drop_width=64, time_stripes_num=2, freq_drop_width=8, freq_stripes_num=2):
        ...
        # Spec augmenter
        self.spec_augmenter = SpecAugmentation(time_drop_width=time_drop_width, time_stripes_num=time_stripes_num,
...
class Cnn10(nn.Module):
    ...
    def forward(self, input, mixup_lambda=None, lengths=None):
        ...
        if lengths is not None:
            x2 = []
            for l, a in zip(lengths, x):
                x2.append(a[..., :math.ceil(x.shape[-1]*l)].mean(dim=1))
            x2 = torch.stack(x2)
        else:
            x2 = torch.mean(x, dim=2)
        x = x1 + x2
...
if __name__ == '__main__':
    ...
    state = torch.load('/home/paul/Downloads/Cnn10_mAP=0.380.pth', map_location=torch.device('cpu'))
